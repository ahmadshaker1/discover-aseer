...
function BreadcrumbChevron({ flip = false }: { flip?: boolean }) {
  return (
    <svg
      width="5"
      height="10"
...
const PageBanner = ({
  ...
  const shouldFlipArrows = breadcrumbDir === "rtl" ? false : !isRtl;
          ...
          >
            {breadcrumbs.map((crumb, index) => (
              <span key={index} className="inline-flex items-center gap-1.5 sm:gap-2">
                ...
                {index < breadcrumbs.length - 1 ? <BreadcrumbChevron flip={shouldFlipArrows} /> : null}
              </span>
            ))}
          </div>
