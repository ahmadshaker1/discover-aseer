...
const EventsFilterSidebar = ({
  ...
  return (
    <div
      className="w-full min-w-[min(100%,280px)] rounded-2xl border border-gray-200 bg-white p-4 sm:p-6 lg:min-w-[300px]"
      dir="rtl"
    >
      ...
      <section className="mb-6 border-b border-gray-200 pb-6">
        ...
        <ul className="flex flex-col gap-1">
          {EVENT_INTEREST_ROWS.map((row) => {
            ...
            return (
              <li key={row.id}>
                <label className="flex cursor-pointer items-center gap-3 rounded-lg py-2 pr-1 transition-colors hover:bg-gray-50">
                  <input
                    type="checkbox"
                    checked={checked}
