...
This folder powers **`/events`**: the catalog grid, hover cards, and the filter sidebar. Today the page passes **`DUMMY_EVENTS`** from `dummyEvents.ts`. Replace that with data from your API and keep rendering typed as **`EventListingItem`** (`types.ts`).
