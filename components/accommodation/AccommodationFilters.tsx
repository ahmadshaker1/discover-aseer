...
const AccommodationFilters = ({
  ...
  return (
    <aside
      className="w-full shrink-0 rounded-2xl border border-gray-200 bg-white p-4 sm:p-6 lg:h-[796px] lg:w-[320px] lg:rounded-none lg:border-y-0 lg:border-l-0 lg:border-r lg:border-[#0000001A] lg:bg-transparent lg:pt-6 lg:pr-8 lg:pl-8 lg:pb-0 lg:shadow-none"
      dir="rtl"
    >
      ...
      <section className="mb-6 border-b border-gray-200 pb-6">
        <div ref={cityWrapRef} className="relative" dir="rtl">
          <button
            ...
            className="relative flex w-full appearance-none items-center gap-2 rounded-full border border-gray-300 bg-white py-3 pl-4 pr-11 text-right text-sm text-black focus:border-[#7300CD] focus:outline-none focus:ring-2 focus:ring-[#7300CD]/20"
          >
            ...
            <span className="flex min-w-0 flex-1 items-center justify-start gap-2">
              <CityPinIcon className="shrink-0" />
              <span className="min-w-0 truncate">{selectedCityLabel}</span>
            </span>
          </button>
          {cityOpen ? (
            <ul
              className="absolute z-30 mt-2 max-h-60 w-full overflow-y-auto rounded-2xl border border-gray-200 bg-white py-1 shadow-lg"
              role="listbox"
              dir="rtl"
            >
              <li>
      ...
      <section dir="rtl" className="mb-6 border-b border-gray-200 pb-6">
        <label className="flex w-full cursor-pointer items-center gap-3 rounded-lg py-2 pr-1 transition-colors hover:bg-gray-50/80">
          <div className="relative shrink-0">
            <input
              type="checkbox"
              checked={onlyExceptional}
              onChange={() => onOnlyExceptionalChange(!onlyExceptional)}
              className="peer h-4 w-4 cursor-pointer appearance-none rounded-[4px] border border-[#0000001A] bg-[#F3F3F5] shadow-[0px_1px_2px_0px_#0000000D] checked:border-[#6027D2] checked:bg-[#6027D2] focus:outline-none focus:ring-2 focus:ring-[#7300CD]/20"
            />
            <svg
