...
const navButtonClass =
  "flex h-8 min-w-[50px] items-center justify-center gap-2 whitespace-nowrap rounded-[8px] px-3 text-center text-[18px] font-bold leading-5 tracking-[-0.15px] text-foreground transition-colors hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2";

const buttonBase =
  "flex h-9 min-w-9 items-center justify-center rounded-full border px-3 text-sm transition";

function CatalogPaginationInner({
  currentPage,
  totalPages,
  ...
  return (
    ...
    >
      {currentPage > 1 ? (
        ...
        <span className={`${navButtonClass} cursor-not-allowed text-muted-foreground`}>
          <span className="inline-flex rtl:rotate-180" aria-hidden>
            <svg
              width="9"
              height="16"
      ...
      {pages.map((page) => {
        ...
        if (isActive) {
          return (
            <span
              key={page}
              className={`${buttonBase} border-primary bg-primary text-primary-foreground`}
              aria-current="page"
            >
              {page}
            </span>
        ...
        return (
          <Link
            key={page}
            href={hrefFor(page)}
            className={`${buttonBase} cursor-pointer border-border bg-surface text-foreground hover:border-primary hover:text-primary`}
            scroll
          >
            {page}
          </Link>
...
export default function CatalogPagination(props: CatalogPaginationProps) {
  return (
    <Suspense fallback={null}>
      <CatalogPaginationInner {...props} />
    </Suspense>
  );
}
