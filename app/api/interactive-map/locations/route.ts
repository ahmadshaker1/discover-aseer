...
import { resolveMapPlaceImageUrl } from "@/lib/maps/resolveMapPlaceImage";
...
const DEFAULT_DIRECTUS_URL = "https://tool-portal.discoveraseer.com";
...
const buildMapPlace = (
  ...
  const imageUrl = resolveMapPlaceImageUrl(row, source, directusUrl);
  ...
  try {
    ...
    for (let i = 0; i < COLLECTIONS.length; i += 1) {
      ...
      const mapped = result.value
        ...
        .map((row, index) =>
          buildMapPlace(row, collection, index, locale, directusUrl),
        );
